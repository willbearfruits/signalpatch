...
| ASan/UBSan runs | **Closed** — full test suite passes under `-fsanitize=address,undefined -fno-sanitize-recover=all` (`build-asan/`; last run 2026-09-15 after the pass-3 fixes, 44 tests, clean) |
| TSan run | **Closed** for the test suite — `build-tsan/` (`-fsanitize=thread`, headless modules) runs all 44 tests with zero reports (2026-09-15, after the pass-3 fixes). The live app (audio callback vs. message thread under a real device) is still not TSan-instrumented. |
