...
Run: `./build/signalpatch_artefacts/RelWithDebInfo/SignalPatch [--board] [--kiosk] [--unmute] [--scale=N] [patch.signalpatch|bundle.zip]` (`--help` explains) — on a PipeWire desktop use `PIPEWIRE_QUANTUM=128/48000 pw-jack ./build/.../SignalPatch` (64 also works but forces the whole graph, i.e. every desktop app on the same interface, to that quantum). `PIPEWIRE_LATENCY` alone is only a hint: pw-jack locks the JACK client to whatever quantum the graph had when it connected (1024 if a browser was playing), `PIPEWIRE_QUANTUM` forces it. Keep the interface on PipeWire's **Pro Audio** profile: a raw ALSA open by JUCE makes WirePlumber drop the capture node, and the first-launch device pick now tries JACK before ALSA and rejects a device that opened with zero outputs. The status bar shows the block size the callback actually receives (pipewire-jack reports its max quantum as the buffer size).
