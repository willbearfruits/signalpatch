...
- Effect nodes are mono; MIDI/OSC mapping, undo/redo, and third-party plug-in hosting are not implemented yet.
...
- Model load/warm-up happens in `NeuralAmpNode::setExtraState` on the message thread; the previous model is retired (never freed by the audio thread). The "NAM model loads and processes" engine test loads a real Boss model from the lv2 clone's `models/` dir and skips gracefully when absent.
- `NEURAL AMP (OFF)` is a safe passthrough placeholder, not fake inference. NAM integration is staged behind `docs/NAM_ROADMAP.md` (planned `SIGNALPATCH_ENABLE_NAM` option and a narrow NAMCore adapter) with explicit latency/CPU gates.
- `docs/PRODUCT.md` is the product target, not a claim of what ships; `README.md` lists what actually works in the 0.1 slice. `docs/PRODUCTION_READINESS.md` tracks which REALTIME_SAFETY gates are verified vs. open — update it when closing a gate (allocation trap, soak test, sanitizers are the big open ones).
